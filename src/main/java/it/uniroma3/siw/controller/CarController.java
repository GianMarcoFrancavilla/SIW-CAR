...
public class CarController {
	...
	public String newCar(@Valid @ModelAttribute("car") Car car,
	    ...
	    try {
	        ...
	        // Salva il brand e il modello associati alla car
	        Brand brand = brandRepository.findById(car.getBrand().getId()).orElseThrow();
	        it.uniroma3.siw.model.Model modelInstance = modelRepository.findById(car.getModel().getId()).orElseThrow();
	        car.setBrand(brand);
	        car.setModel(modelInstance);
	...
	@GetMapping("/filterPageForSale")
	public String filterPageForSale(Model model) {
		...
		return "filterPageForSale.html";
	}
	
	
	@GetMapping("/filterPageNotForSale")
	public String filterPageNotForSale(Model model) {
		model.addAttribute("cars", this.carRepository.findSolds());
		model.addAttribute("brands", this.brandRepository.findAll());
		model.addAttribute("models", this.modelRepository.findAll());
		return "filterPageNotForSale.html";
	}
	...
	public String getCarsForSale(Model model) {
	    ...
	    Map<Long, String> carFirstImages = new HashMap<>();
	    for (Car car : cars) {
	        if (!car.getCarImages().isEmpty()) {
	            CarImage firstImage = car.getCarImages().get(0);
	            String base64Image = Base64Utils.encodeToString(firstImage.getImageData());
	            carFirstImages.put(car.getId(), base64Image);
	        } else {
	            carFirstImages.put(car.getId(), null); // Imposta null se non ci sono immagini
	        }
	    }
	    model.addAttribute("carFirstImages", carFirstImages);
	...
	public String getCarsNotForSale(Model model, Principal principal) {
	    ...
	    List<Car> cars = this.carRepository.findSolds();
	...
	@GetMapping("/admin/carsForSaleAdmin")
	public String getCarsForSaleAdmin(Model model) {
	    List<Car> cars = this.carRepository.findUnsolds();
	    model.addAttribute("cars", cars);
	    ...
	    model.addAttribute("brands", this.brandRepository.findAll());
	    return "admin/carsForSaleAdmin.html";
	}
	...
	@GetMapping("/filteredCarsForSale")
	public String filteredCarsForSale(
		...
		List<Car> filteredCars = carRepository.findUnsoldsByAttributes(
	    ...
	    for (Car car : filteredCars) {
	        if (!car.getCarImages().isEmpty()) {
	            ...
	            // Se non ci sono immagini, puoi impostare un valore predefinito
	            carFirstImages.put(car.getId(), null); // oppure "" o altro valore appropriato
	        }
	    }
	    model.addAttribute("carFirstImages", carFirstImages);
		...
		return "filteredCarsForSalePage";
	}
	
	@GetMapping("/filteredCarsNotForSale")
	public String filteredCars(
		...
		// Esempio di servizio fittizio:
		List<Car> filteredCars = carRepository.findSoldsByAttributes(
				licensePlate, carModel, brand, minYear, maxYear,
				minMileage, maxMileage,minPrice, maxPrice, color);
		...
		// Ritorna il nome della pagina risultante
		return "filteredCarsNotForSalePage";
	}
