...
public interface CarRepository extends CrudRepository<Car, Long> {
	...
	@Query("SELECT c FROM Car c " +
	        ...
	        "AND (:minYear IS NULL OR c.yearOfProduction >= :minYear) " + // Condizione aggiornata
	        "AND (:maxYear IS NULL OR c.yearOfProduction <= :maxYear) " + // Condizione aggiornata
    ...
    List<Car> findUnsoldsByAttributes(@Param("licensePlate") String licensePlate,
    ...
    @Query("SELECT c FROM Car c " +
            ...
            "AND (:maxPrice IS NULL OR c.price <= :maxPrice) " +
            "AND (:color IS NULL OR c.color = :color OR :color = '')")
    List<Car> findSoldsByAttributes(@Param("licensePlate") String licensePlate,
                               @Param("carModel") Model carModel,
                               @Param("brand") Brand brand,
                               @Param("minYear") Integer minYear,
                               @Param("maxYear") Integer maxYear,
